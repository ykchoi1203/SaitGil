...
/**
 * Servlet implementation class ShoppingOrderCompleteServlet
 */
@WebServlet("/ocList.sp")
public class ShoppingOrderCompleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ShoppingOrderCompleteServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int listCount = new ShoppingService().getOrderListCount(userId);
		...
		ArrayList<Order> list = new ShoppingService().getOrderFinish(currentPage, boardLimit, userId);
		...
		request.getRequestDispatcher("views/shopping/shoppingOrderHistory.jsp").forward(request, response);
	}
