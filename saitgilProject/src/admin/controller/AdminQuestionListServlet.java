...
public class AdminQuestionListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int listCount = new AdminService().getInquiryCount();
		...
		ArrayList<Inquiry> list = new AdminService().selectInquiryList(currentPage, boardLimit);
		PageInfo pi = new PageInfo(currentPage, listCount, pageLimit, maxPage, startPage, endPage, boardLimit);
		
		request.setAttribute("pi", pi);
		request.setAttribute("iList", list);
		request.getRequestDispatcher("views/admin/adminQuestionList.jsp").forward(request, response);
	}
